...
def index():
    ...
    item  = { 'label': plugin.get_string(30001), 'path': plugin.url_for('show_featured_events'), 'thumbnail' : 'http://c.thumbs.redditmedia.com/QzaSL6tRjZuybp0J.png' }
    items.append(item)
    item  = { 'label': plugin.get_string(30002), 'path': plugin.url_for('show_learning_center') }
    items.append(item)
    item  = { 'label': plugin.get_string(30003), 'path':plugin.url_for('open_settings')  }
    items.append(item)
...
def show_featured_events():
    ...
    item  = { 'label': plugin.get_string(30004), 'path': plugin.url_for('show_events') }
    show_items.append(item)
    return show_items
...
def show_events():
    ...
    for lolevent in events:
        status = plugin.get_string(30005)
        if (lolevent.status < 99):
            if (lolevent.status == 1):
                status = plugin.get_string(30006)
            if (lolevent.status == 0):
                status = plugin.get_string(30007)
...
def show_matches(eventId, dayId):
    ...
    if (day is not None):
        for match in day.matches:
            ...
            titleFormat = plugin.get_string(30008)
                    ...
                    if (video is not None):
                        ...
                        if (video['text'] is not None and video['videoId'] is not None):
                            if (video['videoId'] is not None and video['videoId'] != 'EMPTY'):
                                youtube_url = PluginUtils.unescape(plugin.get_string(30100)) % video['videoId']
                            else:
                                youtube_url = ""

                            item  = { 'label': video['text'].replace('&amp;', '&') + " @" + video['time'] + "",
                                      'path': youtube_url,
                                      'thumbnail' : plugin.get_string(30100) % video['videoId'],
                                      'is_playable': True }
                            items.append(item)
...
def show_learning_center():
    ...
    item  = { 'label': plugin.get_string(30009), 'path': plugin.url_for('show_leaguecraft101') }
    items.append(item)
...
def show_leaguecraft101():
    ...
    for video in LearningCenter.leaguecraft_videos():
        ...
        item  = { 'label': video['text'],
                  'path': youtube_url,
                  'thumbnail' :plugin.get_string(30101) % video['videoId'],
